...
fn lexer_basics() {
  ...
  let mut lexer = LexerBuilder::new()
    ...
    .ignore(
      ...
      // and use `bind` to bind the action to a sub kind.
      // be ware: the `Anonymous` is NOT the variant of the `MyKind` enum,
      // it is a struct generated by the `#[whitehole_kind]` macro.
      whitespaces().bind(Anonymous),
    )
    // if you want to bind the action to the default sub kind
    ...
    // the first parameter is the target kind,
    ...
    .define(
      ...
      // these actions will have the same target sub kind
      [
